...
def calendar_list(request):
    calendars = Calendar.objects.filter(Q(created_by=request.user)|Q(users__id=request.user.pk))
...
def calendar_new(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            calendar.save()
            form.save_m2m()
            return redirect(reverse('fullcalendar:view_calendar', args=[calendar.id]))
    else:
...
def save_event(request, slug):
    response = {}
    if request.POST:
        if request.POST.get('id_event'):
            e = get_object_or_404(Events, pk=request.POST.get('id_event'))
            form = EventsModelForm(request.POST, instance=e)
        else:
            form = EventsModelForm(request.POST)
        if form.is_valid():
            calendar = get_object_or_404(Calendar, id=slug)
...
def settings_calendar(request, slug):
    ...
    if request.POST:
        ...
        width = 12
    return render(request, "aula_virtual/base_form.html", locals())
